...
model = PB.create_model_from_config(
    joinpath(@__DIR__, "PALEO_examples_romglb_cfg.yaml"), "romglb_P_O2_open",
    modelpars=Dict(
        "matdir"=>joinpath(@__DIR__, "romaniello2010_transport") # assume zip file has been downloaded and unpacked in this subfolder
    )
)

config_ocean_romglb_expts(model, ["baseline"]); tspan=(0, 1e4)
...
plot_ocean_burial_CorgP(paleorun.output; pager=pager)
pager(:newpage) # flush output